  ...
  head: () => ({
    meta: [
      { title: "WhatsCoach — Personal trainer com IA no WhatsApp | R$47/mês" },
      {
        name: "description",
        content:
          "Treino personalizado com IA, acompanhamento diário no WhatsApp e resultado visível. R$47/mês.",
      },
...
const DEMO_CHAT = [
  ...
  {
    from: "coach",
    text: "Seu plano tá pronto 🔥\nDia 1 — Pernas\n• Agachamento 4x12\n• Afundo 3x10\nBora hoje? Vai levar 30min!",
  },
  { from: "user", text: "Feito! 💪" },
  {
    from: "coach",
    text: "Aí sim! Você já tá melhor que semana passada 🚀 Amanhã a gente aumenta!",
  },
];
...
function Header() {
  ...
  return (
    <header
      className={`sticky top-0 z-50 border-b transition-all duration-300 ${scrolled ? "border-border/60 bg-background/95 backdrop-blur-xl shadow-card" : "border-transparent bg-transparent"}`}
    >
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
        ...
        <div className="flex items-center gap-3">
          <Link
            to="/auth"
            className="hidden text-sm text-muted-foreground hover:text-foreground md:block transition-colors"
          >
            Entrar
          </Link>
          <Link to="/auth">
      ...
      <div className="relative mx-auto grid max-w-6xl gap-12 px-4 pb-24 pt-16 md:grid-cols-2 md:items-center md:pb-36 md:pt-24">
        <div className="space-y-6">
          ...
          <h1 className="font-display text-4xl font-extrabold leading-[1.08] tracking-tight sm:text-5xl md:text-6xl">
            Seu personal trainer <span className="text-gradient-primary">no WhatsApp</span>, todo
            dia. 💪
          </h1>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <Link to="/auth" className="contents">
              <Button
                size="lg"
                className="h-14 bg-gradient-primary px-8 text-base font-bold text-primary-foreground shadow-glow hover:opacity-95"
              >
            ...
            <a href="#como-funciona" className="contents">
              <Button
                size="lg"
                variant="outline"
                className="h-14 border-border/60 bg-surface/40 text-base backdrop-blur"
              >
    ...
    <section id="como-funciona" className="mx-auto max-w-6xl px-4 py-20 md:py-28">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-3 text-muted-foreground">
          3 passos. Sem complicação. Resultado de verdade.
        </p>
      </div>
      <div className="mt-14 grid gap-5 md:grid-cols-3">
        {steps.map((s, i) => (
          <div
            key={s.title}
            className="group relative rounded-2xl border border-border bg-card p-7 shadow-card transition-all hover:border-primary/30 hover:shadow-glow"
          >
            <div
              className={`mb-5 inline-flex h-12 w-12 items-center justify-center rounded-2xl ${s.color}`}
            >
...
function Benefits() {
  const items = [
    {
      icon: Timer,
      title: "Treino de 20–40min",
      desc: "Cabe na sua rotina, mesmo nos dias mais corridos.",
      color: "text-primary",
    },
    {
      icon: Dumbbell,
      title: "Em casa ou academia",
      desc: "Adapta ao que você tem disponível agora.",
      color: "text-whatsapp",
    },
    {
      icon: Target,
      title: "IA que aprende com você",
      desc: "Ajusta carga, exercícios e ritmo com base no seu feedback.",
      color: "text-accent",
    },
    {
      icon: Trophy,
      title: "Resultado em semanas",
      desc: "Consistência diária bate perfeição esporádica.",
      color: "text-primary",
    },
    {
      icon: Shield,
      title: "Sem lesão",
      desc: "Exercícios adaptados ao seu nível para evitar dores e lesões.",
      color: "text-whatsapp",
    },
    {
      icon: Users,
      title: "Suporte diário",
      desc: "Seu coach está online todo dia. Basta mandar mensagem.",
      color: "text-accent",
    },
  ];
    ...
    <section className="bg-surface/30 py-20 md:py-28">
      <div className="mx-auto max-w-6xl px-4">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-3 text-muted-foreground">
            Tudo pensado para você manter o ritmo sem precisar de força de vontade.
          </p>
        </div>
        <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item) => (
            <div
              key={item.title}
              className="rounded-2xl border border-border bg-card p-6 transition-all hover:border-primary/20 hover:shadow-card"
            >
              <div
                className={`mb-4 inline-flex h-11 w-11 items-center justify-center rounded-xl bg-surface ${item.color}`}
              >
      ...
      <div className="grid gap-12 md:grid-cols-2 md:items-center">
        <div className="space-y-6">
          ...
          <p className="text-muted-foreground leading-relaxed">
            Sem app pesado, sem planilha. Você só responde uma mensagem por dia e o WhatsCoach cuida
            do resto — treino, motivação e ajustes.
          </p>
          ...
          {TESTIMONIALS.map((t) => (
            ...
            >
              ...
              <div className="mt-5 flex items-center gap-3 pt-5 border-t border-border/40">
                <div
                  className={`grid h-10 w-10 shrink-0 place-items-center rounded-full bg-gradient-to-br ${t.color} text-sm font-bold text-white`}
                >
                  {t.avatar}
                </div>
                <div>
    ...
    <section className="mx-auto max-w-3xl px-4 py-16 text-center">
      <div className="rounded-3xl border-2 border-whatsapp/30 bg-whatsapp/5 p-10">
        ...
        <p className="mt-3 text-muted-foreground leading-relaxed">
          Se em 7 dias você não gostar por qualquer motivo, devolvemos 100% do seu dinheiro. Sem
          perguntas, sem burocracia. Simples assim.
        </p>
      ...
      <div className="relative mx-auto max-w-3xl px-4 text-center space-y-6">
        ...
        <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
          <Link to="/auth">
            <Button
              size="lg"
              className="h-14 bg-gradient-primary px-10 text-base font-bold text-primary-foreground shadow-glow hover:opacity-95"
            >
              Começar agora — R$47/mês <ArrowRight className="ml-2 h-5 w-5" />
            </Button>
          </Link>
